...
RB_fit <- function(D) {
	nls(Difference~RB(C0,C1,C2,C3,C4,C5,Angle), 
		start=list(C0=20,C1=10,C2=0,C3=0,C4=0,C5=0), 
#		weights=(1/(abs(Angle)+0.001)),
		weights=( boltz_invert_kjmol(Angle, Gaussian)  ),
		data=D
	) 
}

gaussian <- read.table("/Users/nicholassiemons/Dropbox/OBT/0064/Thiophene_dihedral_attempt3/DPP.txt", header=FALSE) %>% 
	as_tibble() %>% 
...
rbind(gaussian, MD_scan_off) %>%
	...
	select(Angle, Difference, Gaussian) %>% 
	dplyr::filter(Angle<178) %>%
	RB_fit(.)
