...
gaussian_b3lyp <- read.table("~/Dropbox/OBT/0014/Summary/b3lyp_with_methyl.txt") %>% 
...
Data %>% dplyr::filter(Energy < 22) %>%  
	ggplot(aes(x=Angle, color=Method)) +
		...
		labs(x=expression(phi~'(CBB-CAA-CAA-ST) / '*degree*''), y="E (KJ/mol)")
